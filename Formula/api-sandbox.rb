class ApiSandbox < Formula
  include Language::Python::Virtualenv

  desc "CLI for API sandboxes. Test your APIs in a sandbox env."
  homepage "https://www.digitalapi.ai/"
  url "https://files.pythonhosted.org/packages/d9/d2/b98f22c29ea7e0d35dd87195130939eca37fbc5117df65e64acba9010e35/api_sandbox-0.1.6.tar.gz"
  ...
  def install
    # Public Homebrew installs are customer-only. Drop the internal
    # postgres extra because it pulls wheel-only `psycopg-binary`.
    inreplace "pyproject.toml", /^\s*"psycopg\[binary\]>=3\.3",\n/, ""
    ENV["OPENSSL_DIR"] = Formula["openssl@3"].opt_prefix
    virtualenv_install_with_resources
  end

  test do
    assert_match "Usage", shell_output("#{bin}/sdb --help")
  end
end
